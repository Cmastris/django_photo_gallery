...
8) There won't be much to see at this point, so add some placeholder data (photos, collections etc.) via the Django admin site or programmatically, as detailed in Django's [official tutorial](https://docs.djangoproject.com/en/4.2/intro/tutorial02/). From there, you can make any adjustments to the project code as desired.

## Deployment
When you're ready to deploy a production (i.e. public) version of the website, be sure to read Django's [deployment documentation](https://docs.djangoproject.com/en/4.2/howto/deployment/) (including the [deployment checklist](https://docs.djangoproject.com/en/4.2/howto/deployment/checklist/)) to avoid security vulnerabilities and other issues.

For this project, the following deployment steps will also be necessary:

- Use [prod_urls.py](photo_gallery/photo_gallery/prod_urls.py) (which removes [development-only media file serving](https://docs.djangoproject.com/en/4.2/howto/static-files/#serving-files-uploaded-by-a-user-during-development)) rather than [urls.py](photo_gallery/photo_gallery/urls.py)
