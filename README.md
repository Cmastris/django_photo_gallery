...
4) Create a copy of [database_settings_example.py](photo_gallery/photo_gallery/database_settings_example.py) and rename it to `database_settings.py`. Modify/add to the settings as required (depending on the environment and deployment configuration, if applicable).
5) Create a copy of [email_settings_example.py](photo_gallery/photo_gallery/email_settings_example.py) and rename it to `email_settings.py`. Modify/add to the settings as required (depending on the environment and deployment configuration, if applicable).
6) Navigate into the outer `photo_gallery` directory (which contains `manage.py`) in your chosen command-line interface, which is where you should run all of the following commands, and activate your virtual environment (or use [`uv run`](https://docs.astral.sh/uv/concepts/projects/run/)) if applicable.
7) Set up the project using Django's [official tutorial](https://docs.djangoproject.com/en/5.2/intro/tutorial02/) as a guide (skipping the project and app creation/coding steps, i.e. the stuff that's already in this repository). Specifically, this includes applying the database migrations (creating/modifying your local database tables) using `python manage.py migrate` and creating an admin site user (login details) using `python manage.py createsuperuser`.
